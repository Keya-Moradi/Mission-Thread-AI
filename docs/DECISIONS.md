...
Fix: `seed.ts` now requires an explicit `MISSIONTHREAD_SEED_SCOPE` (`"dev"`, `"test"`, or `"github-actions"`, via the new `resolveSeedScopeTargets()` in `packages/core/src/db-safety.ts`) and resolves it to exactly one target list _before_ looking at `DATABASE_URL` at all — an unset or unrecognized scope fails immediately with a safe, credential-free message, never falling back to a broader default. `scripts/with-destructive-auth.mjs` gained a `--scope=<value>` flag that sets `MISSIONTHREAD_SEED_SCOPE` for the spawned child process only (same mechanism as the authorization flag — never written to a file, never exported to the parent shell). Public commands: `npm run db:seed:dev:destructive` (dev only), `npm run db:reset:test` (test only, unchanged public name — it already only ever accepted `LOCAL_TEST_TARGETS` directly, and now also passes `--scope=test` through to the seed step it spawns internally), and `npm run db:seed:github-actions:internal` (GitHub Actions only, used solely by `.github/workflows/ci.yml`, not a normal local-development command). `db:seed:destructive` (unscoped) no longer exists.
