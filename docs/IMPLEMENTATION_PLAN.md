...
Mirrors `SPEC.md` §19 phase-by-phase. Each phase lists deliverables and its quality gate. Only one phase is authorized at a time — see `CLAUDE.md` hard-stop rules. `continue` authorizes exactly the next phase.

## Phase 0 — Plan (this session)
Deliverables: `CLAUDE.md` and `docs/SPEC.md` placed; architecture, Prisma model, risks, Node LTS, and Phase 1 commands proposed and approved; `docs/IMPLEMENTATION_PLAN.md`, `docs/TASKS.md`, `docs/DECISIONS.md`, `docs/ARCHITECTURE.md` written. No implementation files written.
