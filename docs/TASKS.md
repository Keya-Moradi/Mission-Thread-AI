...
- [x] Playwright: `@playwright/test` added to `apps/web`; `playwright.config.ts` (loads `.env.test`, forces `AI_MODE=mock`, never `missionthread_dev`); `e2e/global-setup.ts` (resets/reseeds the test database via the existing `db:reset:test` command before the suite runs); `e2e/decision-workflow.spec.ts` — one happy-path test: sign in as Program Manager, open the seeded successful analysis, approve one pending mitigation option with a milestone-date proposed change through the real UI, confirm the apply preview, type the exact `APPLY` confirmation, apply, verify the option shows `APPROVED`, the proposed change shows `APPLIED`, the actual milestone date changed, and the audit page shows both `DECISION_RECORDED` and `CHANGES_APPLIED`. `npm run test:e2e` (root) builds then runs the suite. Deliberately not added to `.github/workflows/ci.yml` — Phase 8 owns full CI browser-test expansion.
      **`e2e/global-setup.ts`'s reset-on-every-run behavior was removed in the Phase 5 correction pass below** — `npm run test:e2e` no longer performs a hidden database reset. See that section.
...
## Phase 5 correction pass — integrity and test-safety — done (2026-07-23)

Two confirmed issues fixed: overlapping proposed-change writes within one
approval (order-dependent, silently-discarded results), and a hidden
destructive database reset inside the ordinary Playwright command. Full
disposition is in `docs/DECISIONS.md`'s "(Phase 5 correction)" entries.

- [x] `packages/core/src/approvals/overlap.ts`: `getProposedChangeWriteKeys()` (one `<targetType>:<targetId>:<field>` key per field a client-submitted change actually supplies; `NEW_ACTION` always returns none), `getPersistedProposedChangeWriteKeys()` (the persisted-row equivalent), `findDuplicateWriteKey()` (shared core), `validateNoOverlappingProposedChanges()` and `validateNoOverlappingPersistedProposedChanges()`.
- [x] `recordMitigationDecision()` calls `validateNoOverlappingProposedChanges()` immediately after Zod parsing, before any database access — an overlapping approval batch never opens a transaction, loads a target, or creates any row. Two `MILESTONE_DATE`/`RISK_UPDATE`/`BUDGET_UPDATE` entries writing the same field, and byte-identical duplicate proposals, are all rejected `VALIDATION_ERROR`; disjoint fields on one record, updates to different records, and multiple `NEW_ACTION` entries remain valid together.
- [x] `packages/core/src/approvals/persisted-schemas.ts`: strict, `changeType`-keyed schemas for what a stored `ProposedChange` row must contain (correct `targetRecordType`; matching non-empty `oldValue`/`newValue` key sets restricted to allowlisted fields; valid formats/ranges reusing the existing risk/money/date schemas). `parsePersistedProposedChange()` never throws — every failure is a safe `{ ok: false, message }`.
- [x] `applyApprovedChanges()` now parses every `PENDING` row against these schemas — and re-checks the parsed batch for a stored cross-row overlap — before any stale check or domain mutation. `applyDomainMutation()` was rewritten to operate on the validated shape directly, removing every non-null assertion (`change.targetRecordId!`) and blind cast (`newValue as Record<string, unknown>`, `data as Prisma.RiskUpdateInput`) that previously stood in for a runtime guarantee.
- [x] Removed the hidden reset: deleted `apps/web/e2e/global-setup.ts`, removed `globalSetup` from `playwright.config.ts`. `npm run test:e2e` now only builds and runs the suite against whatever is already in `missionthread_test` — never a reset. New, explicitly self-disclosing combined command `npm run test:e2e:reset:test:destructive` (`db:reset:test && test:e2e`) for when both are actually wanted; its own existing authorization gate is unchanged.
- [x] `e2e/decision-workflow.spec.ts` now wraps its body in a `try`/`finally`: captures `MS-001`'s `currentDate` before mutating it, and on success or failure restores it, deletes the `DECISION_RECORDED`/`CHANGES_APPLIED` audit events it created, deletes its `ProposedChange` row(s) and `Decision` row, and returns the `MitigationOption` to `PENDING` — all via `deleteMany`/`updateMany` so cleanup stays safe even after a partial failure. Imports `prisma` directly from `@missionthread/core` (safe here — Playwright runs test files as plain Node/TypeScript, never through Next.js browser bundling). Verified repeatable: two consecutive `npm run test:e2e` runs after one `npm run db:reset:test` both pass.
- [x] Tests added: `overlap.test.ts` (17 pure tests — write-key derivation, every rejection/allowed-combination case), 9 new tests in `record-decision.test.ts` (overlap rejection for each changeType, duplicate proposals, allowed disjoint/multi-target/multi-NEW_ACTION combinations, and a dedicated no-Decision/no-status-change/no-ProposedChange/no-audit-event proof), 6 new tests in `apply-changes.test.ts` (wrong `targetRecordType`, malformed `oldValue`, malformed `newValue`, key-set mismatch, directly-inserted stored-batch overlap, and a mixed well-formed/malformed batch proving no partial apply) — 32 new tests (548 → 580 core), 15 web unchanged, 595 combined.
- [x] Preserved unchanged: `Decision.mitigationOptionId @unique`, terminal option states, database-role revalidation, Engineering-Lead revision-only permission, Executive-Viewer read-only access, server-generated old values, stale-data protection, exact `APPLY` confirmation, all-or-nothing apply, concurrent decision/apply protection, bounded audit payloads, append-only behavior, `AI_MODE=mock`, all existing smoke checks. No Prisma migration was needed — this pass added application-layer validation only, no new database constraint.
- [x] Docs: `docs/DECISIONS.md` (4 new "(Phase 5 correction)" entries, plus a superseded-notice amendment on the original Playwright entry), `docs/ARCHITECTURE.md` (new "Overlap rejection" and "Apply-time persisted-snapshot revalidation" paragraphs), `README.md`, this file.
- [x] Quality gate: `node -v`, `npm ci`, `db:validate`, `db:generate`, `lint`, `format:check`, `typecheck` (all 3 workspaces), `test`, `build`, `smoke:test` all run first without any destructive operation. Fresh `AskUserQuestion` authorization obtained before the one `db:reset:test` this pass performed; `npm run test:e2e` then run twice in a row afterward with no further reset, both passing, proving the bounded cleanup makes the suite repeatable. Confirmed: no application command touched `missionthread_dev`; no live provider request occurred; no Phase 6 work was added.

## Phase 5 Playwright database-isolation repair — done (2026-07-24)

Confirmed defect fixed: an ambient shell `DATABASE_URL` (e.g. pointed at
`missionthread_dev` for normal local development) could survive into the
Playwright worker process — which statically imported `prisma` from
`@missionthread/core`, constructing a real client from that ambient value
— even though the browser-driven Next.js web server always correctly used
`missionthread_test` (it received the resolved test environment
explicitly). Full disposition is in `docs/DECISIONS.md`'s "Playwright
database-isolation repair" entry.

- [x] `packages/core/package.json` gained an `"exports"` map (`"." : "./src/index.ts"`, `"./db-safety": "./src/db-safety.ts"`) so `db-safety.ts`'s pure helpers can be reused without importing the root barrel (which re-exports `db.ts`'s eagerly-constructed `prisma`). Verified the root `"@missionthread/core"` specifier still resolves everywhere — full production build and complete test suite both pass unchanged after adding it.
- [x] `apps/web/e2e/playwright-test-environment.ts`: `resolvePlaywrightTestEnvironment(ambientEnv, parsedTestEnv)` — pure, side-effect-free — requires `.env.test` to have loaded and defined `DATABASE_URL`, and accepts only `localhost:55432/missionthread_test`, `127.0.0.1:55432/missionthread_test`, or (only when `GITHUB_ACTIONS === "true"`) the GitHub Actions service-container tuple, reusing `findApprovedDatabaseTarget()`/`sanitizeDatabaseUrl()`/`LOCAL_TEST_TARGETS`/`GITHUB_ACTIONS_TEST_TARGETS` directly from `packages/core/src/db-safety.ts` — no second, independently maintained allowlist. `assertPlaywrightTestDatabaseTarget(env)` is the same rejection logic, reusable at any point-of-use.
- [x] `playwright.config.ts` now loads `.env.test` with `override: true` (an ambient `DATABASE_URL` no longer survives), fails immediately if the load errored or the resolved target isn't approved, and applies the one resolved environment via `Object.assign(process.env, ...)` — used identically by this worker process (and everything forked from it) and by `webServer.env`. Added `testMatch: "**/*.spec.ts"` so Playwright never tries to run the new pure `*.test.ts` file itself.
- [x] `e2e/decision-workflow.spec.ts`: removed the static `import { prisma } from "@missionthread/core"`. `getPlaywrightTestPrisma()` calls `assertPlaywrightTestDatabaseTarget(process.env)` — re-verified independently, not trusted from earlier config-loading — then loads `@missionthread/core` (the actual point Prisma gets constructed), then confirms a live `SELECT current_database()` returns exactly `"missionthread_test"`. Called once before `milestoneBefore` is captured and before the `try`/`finally` begins — if it throws, zero queries (including cleanup) ever run; no fallback reset.
- [x] **Real bug found and fixed along the way:** a genuine `await import("@missionthread/core")` for that deferred load reproducibly failed (`SyntaxError: ... does not provide an export named 'GITHUB_ACTIONS_TEST_TARGETS'`) — Playwright compiles spec files to CommonJS with its own `require` transform hook, and a real ESM dynamic `import()` from inside that goes through Node's separate native loader for a second, inconsistent load of the same package graph. Fixed with `require("@missionthread/core")` instead, staying on the one loader already proven to work for the `db-safety` subpath import — reproduced the failure and the fix directly before committing to it.
- [x] `apps/web/vitest.config.ts` extended to include `e2e/**/*.test.ts` alongside `src/**/*.test.ts`, for the new pure test file living next to the module it tests.
- [x] Tests added: `e2e/playwright-test-environment.test.ts` — 19 pure tests covering every acceptance/rejection rule (ambient-dev-vs-.env.test-test, wrong port, missing/malformed/dev/remote-host `DATABASE_URL`, both approved loopback spellings, the GitHub-Actions-tuple-only-inside-a-real-GitHub-Actions-context rule, and that no error message ever contains a username/password/query string/raw URL), including the exact named regression scenario. 15 → 34 web tests; core unchanged at 580. 614 combined.
- [x] Preserved unchanged: client-input and persisted-row overlap rejection, strict persisted snapshot schemas, stale-data protection, all-or-nothing apply, bounded Playwright cleanup, non-destructive ordinary `test:e2e`, the explicitly named destructive convenience command, `Decision.mitigationOptionId @unique`, role revalidation, append-only audit behavior. No Prisma migration was needed.
